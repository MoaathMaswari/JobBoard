<x-layout title="{{ $title }}">
    <button>
        <a class="border-3 border-indigo-500 p-2 m-1 rounded-xl" href="/job/add">Add a Job</a>
    </button>
    <br><br>
    @foreach ($jobs as $job)
        @if ($job->published  )
            ...
            <br>
            <a class="border-3 border-green-500 p-2 m-1 rounded-xl" href="/job/comments/{{ $job->id }}">Read
                Comments</a>
            <a class="border-3 border-yellow-500 p-2 m-1 rounded-xl" href="/job/comment/{{ $job->id }}">Comment</a>
